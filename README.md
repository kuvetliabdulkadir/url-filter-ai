...
Detaylı deploy notları `docs/deploy.md`'de (opsiyonel).
...
- Kod değişikliklerini uygulamak için `docker compose restart` yetmez, `docker compose up --build -d` gerek.
- `tldextract` offline moda alındı (network hang'lerini önlemek için).
- Ollama `0.0.0.0`'da dinlemeli, aksi halde container erişemez.
- Same-origin kontrolü `POST` isteklerinde çalışır (tarayıcı `Origin` gönderir); `GET` için `PUBLIC_READ_TOKEN` şart.
